...
class DetailGunung : AppCompatActivity(), View.OnClickListener {
    ...
    private lateinit var mountainNames : String
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mountainNames = intent.getStringExtra(DATA_NAMA).toString()
        ...
        if(mountainLocation != null){
            setDetailInformation(mountainNames, mountainLocation)
        }
    ...
    override fun onClick(v: View) {
        when(v.id){
            ...
            R.id.btn_look_maps ->{
                val mapIntent = Intent(Intent.ACTION_VIEW, Uri.parse("geo:0,0?q=${mountainNames}"))
